...
import { promoBannerImage } from '../../assets/images';
      ...
      <div className='overflow-hidden relative bg-gradient-to-r rounded-2xl from-accent to-primary'>
        <div className='grid relative z-10 grid-cols-1 gap-8 items-center p-8 md:grid-cols-2 md:p-12'>
          <div className='space-y-6 text-white'>
            <span className='inline-block px-3 py-1 text-sm font-medium text-white rounded-full backdrop-blur-sm bg-white/20'>
              Limited Time Offer
            </span>
            <Title
              level={2}
              ...
              {!user && (
                <Link to='/register'>
                  <Button
                    type='primary'
                    size='large'
                    className='!bg-white !text-accent !border-white !font-medium !min-w-[120px] hover:!bg-white/90 !shadow-md hover:!shadow-lg transition-all duration-300'
                  >
                    Join Now
                  </Button>
                </Link>
